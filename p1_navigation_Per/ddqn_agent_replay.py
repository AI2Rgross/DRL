...
class dqn_replay_Agent():
    ...
    def wmse_loss(Q_expected,Q_targets,weights):
        return [((Q_expected[i]-Q_targets)**2)*weights[i] for i in range(len(ind))].mean(0)
    ...
    def learn(self, experiences, gamma, weights):
        ...
        loss = self.wmse_loss(Q_expected, Q_targets)
...
class dqn_replay_ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        e = self.experience(state, action, reward, next_state, done,self.error)
        self.memory.appendleft(e)
        print("mem len test",self.memory.len()-1,len(self.memory)-1)
        self.total+=self.memory[self.memory.len()-1].error
    ...
    def rem(self):
        ...
        ind=ind[0]
        self.total-=self.memory[ind].error
    ...
    def update(self,losses,ind):
        if(ind != []):     
            j=0 
            for i in ind:
                e=(abs(losses[j])+1e-4)**self.alpha    
                self.total-=self.memory[i].error
    ...
    def weight(N,Pi,alpha,beta)
        return Pi(1./(N*Pi))**beta
    ...
    def distribution(self,uniform=True):
        ...
        if(uniform):
            ...
            ind = np.random.choice(a=np.arange(N), size=self.batch_size,replace=False,p=distrib)
            weights=[self.weight(N,distrib[i],self.alpha,self.beta) for i in ind]               
        return ind,weights
